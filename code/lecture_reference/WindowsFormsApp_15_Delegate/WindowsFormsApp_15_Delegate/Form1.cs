...
namespace WindowsFormsApp_15_Delegate
{
     ...
     *   
     * [참고]
     * ㄴ 내부적으로 Invoke 리스트(= 메서드 리스트 형태)로 구성되어 있음. (TMI)
     * ㄴ _invocationList = [ MethodA, MethodB, MethodC ]; << 이런 식으로 리스트 생성
     * ㄴ Invoke 리스트
     * ㄴ 델리게이트 내부에서 연결된 메서드를 저장해두는 배열(리스트)이며, del() 이라고 호출 사실상 foreach 돌면서 Invoke()를 다 실행하는 구조
     * [참고 끝]
     */
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
             ...
             * [참고] Q) 타입인데 객체를 만든다?
             * A) 
             * 델리게이트는 클래스처럼 '사용자 정의 타입' 임. - 참조형 타입!
             * 즉, delegate로 선언한 것은 "특정한 메서드를 담을 수 있는 클래스"를 만드는 것과 동일한 효과. 
             */
            ...
            /*
             * 내장 델리게이트 타입 (C# 제공)
             * - C#이 자주 쓰는 함수 형태(매개변수 + 반환값)를 위해 특정 표준 델리게이트 타입을 미리 정의 해놓은 것. 
             * 
             * Why?
             * - 간결하고 범용적인 함수 형태를 빠르게 표현할 수 있어서
             * 
             * When?
             * - 간단한 함수, 반복적으로 많이 쓰이는 형태라면 내장 델리게이트 타입!
             * - 의미 있는 이름을 붙이고 복잡한 구조로 사용자 정의를 원한다면 직접 delegate 선언!
             * 
             * [종류]
             * - Func<...>          - 반환 값 (o) / 반드시 마지막 타입이 반환형
             * - Action<...>        - 반환 값 (x) / void
             * - Predicate<...>      - bool 반환하는 특수한 Func (조건 필터리용)
             * 
             * [구조]
             * Func<T1, T2, ..., TResult>
             * - 앞쪽은 매개변수 타입들
            ...
            //int result = Add(3, 5);  // 호출은 되지만 Add를 변수에 담을 순 없음

            /*
             * [참고 2]
             * Q2) 람다식은 꼭 델리게이트와 함께 서야하나요?
             * A)
             * 람다식은 반드시 "델리게이트 타입"에 할당하거나, 델리게이트로 처리되는 구조에서만 사용할 수 있습니다
             * 람다식은 **“이름 없는 함수(익명 함수)”**이기 때문에, 
             * 어떤 타입(그릇)에 담지 않으면 사용할 수 없습니다.
             */
        }
    }
}
