...
// #0. 네임스페이스 설정
// - 클래스들을 하나의 그룹으로 묶어주는 역할 // 그룹화 개념
// - 같은 네임스페이스 안에 있는 클래스끼리는 쉽게 접근 가능. // 같은 네임스페이스로 묶어주면 관련된 것으로 취급
namespace WindowsFormsApp_07_Class
{
    ...
    internal class Car
    {
        ...
        // --- #2-2. readonly 필드
        // - readonly 키워드 사용!
        // - 초기화한 이후 값을 바꿀 수 없는 필드! (생성자에서는 값을 한번 정할 수 있음)
        // - 생성자에게 한 번 설정하면 이후 변경 불가 (불변 데이터용)
        // (참고) Why?
        // - 한번 설정되면 절대 바뀌지 않아야 하는 값을 보호하고 싶을 때!
        ...
        #region 3. 속성
        /*
         * 속성 = 필드에 안전하게 접근할 수 있는 통로
         * 필드에 직접 접근하는 것을 막고, 값을 읽거나 쓸 때 조건 검사 가능.
         */
        ...
        */
        ...
        #region 5. 소멸자
        /*
        * 소멸자 = 객체가 메모리에서 삭제될 때 자동으로 호출되는 특별한 메서드
